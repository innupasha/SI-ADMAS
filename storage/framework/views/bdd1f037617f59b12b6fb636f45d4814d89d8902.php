<?php $__env->startSection('body'); ?>
    ...
    <div class="bg-white border-2 border-black rounded-md p-5 mx-3 my-10">
        ...
        <form id="formAkun" action="/akun/update" method="POST" class="flex flex-col gap-10">
            ...
            <div id="lvl" class="<?php echo e(Auth::user()->lvl == 'masyarakat' ? 'hidden' : ''); ?>">
                <label for="inputLvl">Jabatan</label>
                <input id="inputLvl" name="nik" disabled class="disabled:bg-slate-200 disabled:text-slate-500 rounded-md w-full" type="text" name="lvl" required value="<?php echo e(ucfirst($akun->lvl)); ?>">
            </div>
            ...
            <div class="sm:flex gap-5">
                <div id="nik" class="flex gap-5 flex-1">
                    <div class="flex-1 mb-10 sm:mb-0">
                        ...
                        <input id="inputNik" name="nik" disabled class="disabled:bg-slate-200 disabled:text-slate-500 rounded-md w-full" type="number" name="nik" required value="<?php echo e($akun->nik); ?>">
                    </div>
                    
                    <div id="username" class="flex-1">
                        <label for="inputUsername">Username</label>
                        <input id="inputUsername" name="username" disabled class="disabled:bg-slate-200 disabled:text-slate-500 rounded-md w-full" type="text" name="username" required value="<?php echo e($akun->username); ?>">
                    </div>
                </div>
          
                <div class="flex-1">
                    <label for="inputName">Nama Lengkap</label>
                    <input id="inputName" name="name" disabled class="disabled:bg-slate-200 disabled:text-slate-500 rounded-md w-full" type="text" name="name" required value="<?php echo e($akun->name); ?>">
                </div>
            </div>
            ...
            <div class="sm:flex gap-5">
                <div class="flex-1 mb-10 sm:mb-0">
                    ...
                    <input id="inputEmail" name="email" disabled class="disabled:bg-slate-200 disabled:text-slate-500 rounded-md w-full" type="email" name="email" required value="<?php echo e($akun->email); ?>">
                </div>

                <div class="mb-5 flex-1">
                    <label for="inputTelp">No. Telepon</label>
                    <input id="inputTelp" name="telp" disabled class="disabled:bg-slate-200 disabled:text-slate-500 rounded-md w-full" type="text" name="telp" required value="<?php echo e($akun->telp); ?>">
                </div>
            </div>
        ...
        <?php if($title == 'Detail Pengguna'): ?>
            <div>
                <div class="mx-5 sm:mx-10">
                    <form class="sm:flex items-center gap-2" action="/admin/user/detail/<?php echo e($akun->nik); ?>" method="GET">
                        <div class="sm:flex items-center gap-5">
                            ...
                            <div class="">
                                ...
                                <div class="sm:ml-auto flex gap-3 my-3">
                                    ...
                                    <a href="/admin/user/detail/<?php echo e($akun->nik); ?>/export?tgl_awal=<?php echo e($tgl_awal); ?>&tgl_akhir=<?php echo e($tgl_akhir); ?>&search=<?php echo e($search); ?>&status=<?php echo e($status); ?>" data-tooltip-target="tooltip-export" class="bg-green-500 btn btn-sm sm:mt-0"><i class="fa-solid fa-file-excel"></i></a>
                                    ...
                                    <a data-tooltip-target="tooltip-reset" href="/admin/user/detail/<?php echo e($akun->nik); ?>" class="bg-red-600 btn btn-sm sm:mt-0"><i class="fa-solid fa-arrow-rotate-left"></i></a>        
            ...
            <div class="grid grid-flow-row grid-cols-1 sm:grid-cols-4 gap-3 sm:gap-7 w-full p-5 sm:px-10">
                <?php $__currentLoopData = $pengaduan; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $i): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="card card-compact w-full bg-base-100 shadow-xl box-border border-black border hover:scale-110 hover:z-50 transition-transform overflow-hidden">
                        <figure class="bg-gray-300"><img class="h-[150px] sm:h-[150px] w-full object-contain" src="<?php echo e($i->foto !== NULL ? (asset('img_pengaduan/' . $i->foto)) : asset('img/kosong.jpg')); ?>" alt="img_pengaduan" /></figure>
                        <div class="p-3">
                            <h2 class="font-bold first-letter:uppercase text-sm truncate"><?php echo e($i->judul); ?> <br class="sm:hidden"> <span class="absolute top-0 right-0 <?php echo e($status->Color($i)); ?> p-1 rounded-tl rounded-bl text-white font-semibold"><?php echo e($status->Text($i)); ?></span></h2>
                            <div class="grid grid-flow-row grid-cols-2 gap-3 my-3">
                                <p class="text-sm">Tanggal Kejadian:<br> <?php echo e(date('d-m-Y',strtotime($i->tgl_pengaduan)) ?? 'NULL Tanggal'); ?></p>
                                <p class="text-sm truncate">Lokasi:<br> <?php echo e($i->kecamatan[0]->kecamatan ?? 'NULL Lokasi'); ?></p>
                            </div>
                            <p class="truncate text-sm">Dibuat oleh:<br> <?php echo e($i->user->username ?? 'NULL Username'); ?></p>
                            <div class="card-actions justify-end mt-3">
                                <a href="/pengaduan/me/<?php echo e($i->id); ?>" class="btn btn-sm w-full btn-primary">Detail</a>
                            </div>
                        </div>
                    </div>
...
<?php echo $__env->make('layouts/base', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp4\htdocs\admas\resources\views/admas/akun.blade.php ENDPATH**/ ?>